...
class lC_Products_Admin {
    ...
    if ( $data['has_children'] == '1' ) {
      ...
      while ( $QmultiSKUProducts->next() ) {
        $variants_array[$QmultiSKUProducts->valueInt('products_id')]['data'] = array('cost' => $QmultiSKUProducts->value('products_cost'),
		                                                                                 ...
		                                                                                 'tax_class_id' => $QmultiSKUProducts->valueInt('products_tax_class_id'),'shipping_rule_id' => $QmultiSKUProducts->valueInt('shipping_rule_id'),
  ...
  */
  public static function save($id = null, $data) {
    ...
    if ( is_numeric($id) ) {
      $Qproduct = $lC_Database->query('update :table_products set parent_id = :parent_id, products_quantity = :products_quantity, products_cost = :products_cost, products_price = :products_price, products_msrp = :products_msrp, products_model = :products_model, products_sku = :products_sku, products_weight = :products_weight, products_weight_class = :products_weight_class, products_status = :products_status, groups_pricing_enable = :groups_pricing_enable, qpb_pricing_enable = :qpb_pricing_enable, specials_pricing_enable = :specials_pricing_enable, products_tax_class_id = :products_tax_class_id, products_last_modified = now(), products_sort_order = :products_sort_order, shipping_rule_id = :shipping_rule_id where products_id = :products_id');
      $Qproduct->bindInt(':products_id', $id);
    } else {
      $Qproduct = $lC_Database->query('insert into :table_products (parent_id, products_quantity, products_cost, products_price, products_msrp, products_model, products_sku, products_weight, products_weight_class, products_status, products_tax_class_id, products_ordered, products_date_added, groups_pricing_enable, qpb_pricing_enable, specials_pricing_enable, products_sort_order) values (:parent_id, :products_quantity, :products_cost, :products_price, :products_msrp, :products_model, :products_sku, :products_weight, :products_weight_class, :products_status, :products_tax_class_id, :products_ordered, :products_date_added, :groups_pricing_enable, :qpb_pricing_enable, :specials_pricing_enable, :products_sort_order,:shipping_rule_id)');
    ...
    $Qproduct->bindInt(':shipping_rule_id', $data['shipping_rule_id']);   
  ...
  public static function copy($id, $category_id, $type) {
    ...
    if ( $type == 'link' ) {
      ...
      if ( $Qproduct->numberOfRows() === 1 ) {
        ...
        $Qnew->bindInt(':shipping_rule_id', $Qproduct->valueInt('shipping_rule_id'));
