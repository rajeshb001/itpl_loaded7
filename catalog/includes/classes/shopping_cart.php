...
//          }
  ...
  public function getBillingTermsName($terms_id) {
   ...
   public function shippingrule($product_id){
		//echo $product_id;exit;
		global $lC_Database;
		$Qpshiprule = $lC_Database->query('select shipping_rule_id from :table_products where products_id = :products_id');
		$Qpshiprule->bindTable(':table_products', TABLE_PRODUCTS);
		$Qpshiprule->bindInt(':products_id', $product_id);
		$Qpshiprule->execute();
		$shipping_rule_id = $Qpshiprule->value('shipping_rule_id');
		
		$Qshiprule = $lC_Database->query('select * from :table_shipping_restrict where rule_id = :rule_id');
		$Qshiprule->bindTable(':table_shipping_restrict', TABLE_SHIPPING_RESTRICT);
		$Qshiprule->bindInt(':rule_id', $shipping_rule_id);
		$Qshiprule->execute();
		$shipping_name = $Qshiprule->value('shipping_name');
		return $shipping_name;
	} 
   public function shippingruleid($product_id){
		global $lC_Database;
		$Qpshiprule = $lC_Database->query('select shipping_rule_id from :table_products where products_id = :products_id');
		$Qpshiprule->bindTable(':table_products', TABLE_PRODUCTS);
		$Qpshiprule->bindInt(':products_id', $product_id);
